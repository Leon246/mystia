...
double PairEAM::singleembedding(int im, int itypem, double *coord)
{
  int i,j,m,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz;
  double rsq,p,dphi,rho0,rho1;
  double *coeff,coord0[3];
  double cutoff = cutmax;
  double cutoffsq;
  int tflag = 0;
  double **x = atom->x;
  int *type = atom->type;
  ...
  if (tflag) {
    ...
    // MCMD, larger cutoff
    cutoffsq = (cutoff+2.0)*(cutoff+2.0);
  }
  ...
  for (i = 0; i < nall; i++) {
    ...
    for (j = 0; j < nall; j++) {
      ...
      if (rsq < cutforcesq) {
        ...
        p = MIN(p,1.0);
        coeff = rhor_spline[type2rhor[jtype][itype]][m];
        rho0 += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
        
        // modified rho
        if (j == im) {
          if (tflag) {
            // type change
            coeff = rhor_spline[type2rhor[itypem][itype]][m];
          } else {
            // MCMD
          ...
          if (tflag) {
            // type change
            coeff = rhor_spline[type2rhor[jtype][itypem]][m];
          } else {
            // MCMD
            // recalculate distance
            delx = x[j][0] - coord[0];
            ...
            p -= m;
            p = MIN(p,1.0);
            coeff = rhor_spline[type2rhor[jtype][itype0]][m];
          }
        }
        rho1 += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
    ...
    dphi -= ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
    
    // after
    itype = i == im ? itypem : itype;
    p = rho1*rdrho + 1.0;
    m = static_cast<int> (p);
    m = MAX(1,MIN(m,nrho-1));
    p -= m;
